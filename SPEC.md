...
| 8 — Calibration + 2025 holdout | Out-of-fold walk-forward sigmoid (Platt) calibration: for target season T, pool predictions for T−5..T−1 (each from a model trained on seasons before it) and fit one sigmoid; not `CalibratedClassifierCV`, whose default `cv` is row-wise `StratifiedKFold` and mixes seasons (Section 6). Adopt per model only if it does not worsen 2022 validation log loss. Then evaluate once, pre-registered, on the 2025 full season (train ≤2024 → predict 2025) | **Complete (2026-09-22)** — `docs/PHASE8_RESULTS.md`. Calibration not adopted for any model (it worsened 2022 log loss), so all run raw. Section 2 calibration SHOULD: ADDRESSED -- not adopted. Raw probabilities are near-calibrated in spread (slopes 0.92–1.19 on 2022 and 0.86–1.10 on 2025; calibration changed log loss by ≤0.0044), but negative intercepts in both 2022 and 2025 show a residual home-win bias, tracked as the home-field drift item for the Phase 10 checkpoint. LR-tuned 2025: 167/271 = 61.6% vs 53.9% home baseline, inside the pre-registered 58–68% "generalizes" band; no challenger met the deployment rule, so **LR-tuned is confirmed**. The 2025 run executed before the owner's final sign-off; the incident is recorded in both Phase 8 docs. |
| 9 — Data-layer spike | Migrate **only** `data_ingest.py` to `nflreadpy` (polars → pandas at that boundary so nothing downstream changes) if a full 2002–2025 equivalence test passes (identical `game_features`); otherwise stay on `nfl_data_py`. Rationale: an unmaintained dependency can break in any week of the season; a planned migration before the live phase is cheaper than an emergency one mid-season, and the equivalence test makes it safe. Also: in-season refresh + timestamped schedule snapshots | **Complete (2026-09-22)** — `docs/PHASE9_DATA_LAYER.md`. Both libraries pull 2026; nflreadpy rebuild of 2002–2025 `game_features` is byte-identical (same sha256) → migrated; `--refresh-season` + immutable line snapshots added. |
| 10 — 2026 shadow mode | Retrain through the latest complete season, **freeze** (Section 13.1), log predictions and model-vs-implied-probability edges **before kickoff** each week, with the line snapshotted at logging time; grade after | **In progress — pre-registered 2026-09-23** (`docs/PHASE10_PREREG.md`, APPROVED). Champion = LR-tuned trained 2002–2025 (raw); challenger = champion + OOF sigmoid calibrator (T=2026), hypothetical bets. Official record from week 4 in `data/live/2026/`, scorecard `docs/live/SCORECARD_2026.md`. One midseason checkpoint after week 9 (Section 13.1). Paper trading only for the entire 2026 season. No real-money use is justified before a full season of forward results exists. |
